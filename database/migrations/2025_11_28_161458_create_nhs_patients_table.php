...
return new class extends Migration {
    public function up(): void {
        // Safety guard in case the table was partially created before a failed migration
        if (Schema::hasTable('nhs_patients')) {
            Schema::drop('nhs_patients');
        }
        Schema::create('nhs_patients', function (Blueprint $t) {
            $t->id();

            $t->unsignedBigInteger('application_id')->nullable()->index();
            // Intentionally no foreign key to avoid cross-DB constraint issues

            // same core fields as application
            $t->string('first_name');
            $t->string('last_name');
            $t->date('dob')->nullable();
    ...
    public function down(): void {
        Schema::dropIfExists('nhs_patients');
    }
};