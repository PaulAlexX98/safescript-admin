        ...
        return $table
            ...
            ->columns([
                ...
                Tables\Columns\TextColumn::make('items_summary')
                    ...
                    ->getStateUsing(function ($record) {
                        ...
                        $items = collect($candidates)->first(fn ($c) => is_array($c) && count($c)) ?? [];
                        ...
                        foreach ($items as $it) {
                            ...
                            $opt = data_get($it, 'variations')
                                ?: data_get($it, 'variation')
                                ?: data_get($it, 'optionLabel')
                                ?: data_get($it, 'variant')
                                ?: data_get($it, 'dose')
                                ?: data_get($it, 'strength')
                                ?: data_get($it, 'option');
                        ...
                        $labels = array_values(array_filter(array_map(fn ($v) => trim((string) $v), $labels)));

                        if (empty($labels)) {
                            return '—';
                        }

                        $remaining = max(0, count($items) - count($labels));
                        $text = implode(' | ', $labels);
                ...
                Tables\Columns\TextColumn::make('customer')
                    ...
                    ->getStateUsing(function ($record) {
                        ...
                        $first = data_get($meta, 'firstName')
                            ...
                            ?: data_get($meta, 'patient_snapshot.first_name')
                            ?: optional($record->user)->first_name;

                        $last = data_get($meta, 'lastName')
                            ?: data_get($meta, 'last_name')
                            ?: data_get($meta, 'patient.lastName')
                            ?: data_get($meta, 'patient.last_name')
                            ?: data_get($meta, 'patient_snapshot.last_name')
                        ...
                        return $name !== ''
                            ? $name
                            : (optional($record->user)->name ?: data_get($meta, 'patient.name') ?: data_get($meta, 'name') ?: '—');
                    })
                    ->searchable(query: function (Builder $query, string $search): Builder {
                        $like = '%' . $search . '%';

                        return $query->where(function (Builder $q) use ($like) {
                            $q->orWhere('reference', 'like', $like)
                                ->orWhereRaw("JSON_UNQUOTE(JSON_EXTRACT(meta, '$.firstName')) LIKE ?", [$like])
                ...
                Tables\Columns\TextColumn::make('phone')
                    ...
                    ->getStateUsing(function ($record) {
                        ...
                        return data_get($meta, 'phone')
                            ?: data_get($meta, 'patient.phone')
                            ?: data_get($meta, 'patient_snapshot.phone')
            ...
            ->recordActions([
                Action::make('viewOrder')
                                                ...
                                                ->state(function ($record) {
                                                    ...
                                                    $formatDateTime = function ($dt) {
                                                        ...
                                                        try {
                                                            ...
                                                            return is_scalar($dt) ? trim((string) $dt) : null;
                                                        }
                                                    };
                                                    ...
                                                    $extractAppointment = function (array $meta) use ($formatDateTime, $formatDateOnly) {
                                                        $start = data_get($meta, 'appointment_start_at')
                                                            ?: data_get($meta, 'appointment.start_at')
                                                            ?: data_get($meta, 'appointment.start')
                                                            ?: data_get($meta, 'appointment_at')
                                                            ?: data_get($meta, 'appointment.datetime')
                                                            ?: data_get($meta, 'appointment_date_time')
                                                            ?: data_get($meta, 'booking_datetime')
                                                            ?: data_get($meta, 'selected_slot')
                                                            ?: data_get($meta, 'slot');

                                                        $end = data_get($meta, 'appointment_end_at')
                                                            ?: data_get($meta, 'appointment.end_at')
                                                            ?: data_get($meta, 'appointment.end');
                                                        ...
                                                        $date = data_get($meta, 'appointment_date')
                                                            ?: data_get($meta, 'appointment.date')
                                                            ?: data_get($meta, 'booking_date')
                                                            ?: data_get($meta, 'selected_date')
                                                            ?: data_get($meta, 'date');

                                                        $time = data_get($meta, 'appointment_time')
                                                            ?: data_get($meta, 'appointment.time')
                                                            ?: data_get($meta, 'booking_time')
                                                            ?: data_get($meta, 'selected_time')
                                                            ?: data_get($meta, 'time');

                                                        $formattedDate = $formatDateOnly($date);
                                                        $formattedTime = is_scalar($time) ? trim((string) $time) : null;
                                                    ...
                                                    $meta = $decodeMeta($record->meta ?? []);
                                                    $value = $extractAppointment($meta);
                                                    ...
                                                    try {
                                                        $order = Order::query()
                                                            ->where('reference', (string) ($record->reference ?? ''))
                                                            ->latest('id')
                                                            ->first();

                                                        if ($order) {
                                                            $orderMeta = $decodeMeta($order->meta ?? []);
                                                            $value = $extractAppointment($orderMeta);
                                                            if ($value) {
                                                                return $value;
                                                            }
                                                        }
                    ...
                    ->color('danger')
                    ->requiresConfirmation()
                    ->modalHeading('Delete this walk-in record?')
                    ->modalDescription('This will permanently delete this walk-in record from the list.')
                    ->successNotificationTitle('Walk-in record deleted'),
            ])
            ->toolbarActions([]);
    }
}
