...
namespace App\Filament\Resources\NhsPending;
...
use App\Filament\Resources\NhsPending\Pages\ListNhsPending;
use App\Filament\Resources\NhsPending\Schemas\NhsPendingForm;
...
class NhsPendingResource extends Resource
{
    ...
    protected static ?string $modelLabel = 'NHS Pending';
    ...
    public static function getEloquentQuery(): Builder
    {
        ...
        try {
            if (\Schema::hasColumn((new \App\Models\NhsPending())->getTable(), 'status')) {
                ...
                $query->where(function (Builder $q) {
                    $q->whereNull(DB::raw("JSON_UNQUOTE(JSON_EXTRACT(meta, '$.nhs_pending_status'))"))
                      ->orWhereRaw("LOWER(JSON_UNQUOTE(JSON_EXTRACT(meta, '$.nhs_pending_status'))) = 'pending'");
                });
            }
        } catch (\Throwable $e) {
            // If schema lookup fails for any reason, fall back to meta.nhs_pending_status
            $query->where(function (Builder $q) {
                $q->whereNull(DB::raw("JSON_UNQUOTE(JSON_EXTRACT(meta, '$.nhs_pending_status'))"))
                  ->orWhereRaw("LOWER(JSON_UNQUOTE(JSON_EXTRACT(meta, '$.nhs_pending_status'))) = 'pending'");
            });
        ...
        $query->where(function (Builder $q) {
            $q
                ...
                ->orWhereRaw("LOWER(JSON_UNQUOTE(JSON_EXTRACT(meta, '$.service'))) LIKE '%pharmacy first%'")
                ->orWhereRaw("LOWER(JSON_UNQUOTE(JSON_EXTRACT(meta, '$.serviceName'))) LIKE '%pharmacy first%'")
                ->orWhereRaw("LOWER(JSON_UNQUOTE(JSON_EXTRACT(meta, '$.title'))) LIKE '%pharmacy first%'");
        });
    ...
    {
        return NhsPendingForm::configure($schema);
    }
            ...
            ->columns([
                ...
                TextColumn::make('order_item')
                    ...
                    ->getStateUsing(function ($record) {
                        ...
                        // Helper: turn a line item into "2 × Name" (no variation/options)
                        ...
                        if (empty($labels)) {
                            ...
                            if ($qty === null) $qty = data_get($meta, 'quantity');
                            if ($qty === null) $qty = data_get($meta, 'selectedProduct.qty');
                            if ($qty === null) $qty = data_get($meta, 'selected_product.qty');
                            if ($qty === null) $qty = data_get($meta, 'order.qty');
                            if ($qty === null) $qty = data_get($meta, 'order.quantity');
                            if ($qty === null) $qty = 1;
                ...
                TextColumn::make('customer')
                    ...
                    ->getStateUsing(function ($record) {
                        ...
                        foreach ([
                            ...
                            $v = data_get($meta, $k);
                            if (is_scalar($v) && trim((string) $v) !== '') { $first = (string) $v; break; }
                        }
                        ...
                        foreach ([
                            ...
                            $v = data_get($meta, $k);
                            if (is_scalar($v) && trim((string) $v) !== '') { $last = (string) $v; break; }
                        }

                        // Prefer related user if present
                        if ((!$first || !$last) && isset($record->user)) {
                            $first = $first ?: ($record->user->first_name ?? null);
                            $last  = $last  ?: ($record->user->last_name  ?? null);
                        }

                        // If the pending row doesn't have user_id, fall back to the real order by reference
                        if ((!$first || !$last) && ($record->reference ?? null)) {
                            try {
                                $order = \App\Models\Order::where('reference', $record->reference)->latest()->first();
                                if ($order && isset($order->user)) {
                                    $first = $first ?: ($order->user->first_name ?? null);
                                    $last  = $last  ?: ($order->user->last_name  ?? null);
                                }
                            } catch (\Throwable $e) {
                            }
                        }
                ...
                Action::make('viewOrder')
                                    ...
                                    Grid::make(2)->schema([
                                        TextEntry::make('patient_first_name')
                                            ...
                                            ->state(function ($record) {
                                                ...
                                                if (isset($record->user)) return $record->user->first_name ?? null;
                                                try {
                                                    $order = \App\Models\Order::where('reference', $record->reference)->latest()->first();
                                                    return $order?->user?->first_name ?? null;
                                                } catch (\Throwable $e) {
                                                    return null;
                                                }
                                            }),
                                        TextEntry::make('patient_last_name')
                                            ...
                                            ->state(function ($record) {
                                                ...
                                                if (isset($record->user)) return $record->user->last_name ?? null;
                                                try {
                                                    $order = \App\Models\Order::where('reference', $record->reference)->latest()->first();
                                                    return $order?->user?->last_name ?? null;
                                                } catch (\Throwable $e) {
                                                    return null;
                                                }
                                            }),
                                        TextEntry::make('dob')
                                            ...
                                            ->state(function ($record) {
                                                ...
                                                if (isset($record->user)) return $record->user->dob ?? null;
                                                try {
                                                    $order = \App\Models\Order::where('reference', $record->reference)->latest()->first();
                                                    return $order?->user?->dob ?? null;
                                                } catch (\Throwable $e) {
                                                    return null;
                                                }
                                            })
                                        ...
                                        TextEntry::make('meta.email')
                                            ...
                                            ->state(function ($record) {
                                                ...
                                                if (is_scalar($email) && trim((string) $email) !== '') return (string) $email;

                                                if (isset($record->user) && is_scalar($record->user->email ?? null) && trim((string) $record->user->email) !== '') {
                                        ...
                                        TextEntry::make('meta.phone')
                                            ...
                                            ->state(function ($record) {
                                                ...
                                                $phone = data_get($meta, 'phone')
                                                    ?: data_get($meta, 'user.phone')
                                                    ?: data_get($meta, 'user.phone_number')
                                                    ?: data_get($meta, 'patient.phone')
                                                    ?: data_get($meta, 'patient.phone_number')
                                                    ?: data_get($meta, 'customer.phone')
                                                    ?: data_get($meta, 'customer.phone_number');

                                                if (is_scalar($phone) && trim((string) $phone) !== '') return (string) $phone;

                                                if (isset($record->user) && is_scalar($record->user->phone ?? null) && trim((string) $record->user->phone) !== '') {
                                                    return (string) $record->user->phone;
                                                }
                                        ...
                                        TextEntry::make('patient_address')
                                            ...
                                            ->state(function ($record) {
                                                ...
                                                $addr1 = data_get($meta, 'address1')
                                                    ?: data_get($meta, 'address_1')
                                                    ?: data_get($meta, 'user.address1')
                                                    ?: data_get($meta, 'user.address_1')
                                                    ?: data_get($meta, 'patient.address1')
                                                    ?: data_get($meta, 'patient.address_1')
                                                    ?: data_get($meta, 'customer.address1')
                                                    ?: data_get($meta, 'customer.address_1');

                                                $addr2 = data_get($meta, 'address2')
                                                    ?: data_get($meta, 'address_2')
                                                    ?: data_get($meta, 'user.address2')
                                                    ?: data_get($meta, 'user.address_2')
                                                    ?: data_get($meta, 'patient.address2')
                                                    ?: data_get($meta, 'patient.address_2')
                                                    ?: data_get($meta, 'customer.address2')
                                                    ?: data_get($meta, 'customer.address_2');

                                                $city = data_get($meta, 'city')
                                                    ?: data_get($meta, 'town')
                                                    ?: data_get($meta, 'user.city')
                                                    ?: data_get($meta, 'user.town')
                                                    ?: data_get($meta, 'patient.city')
                                                    ?: data_get($meta, 'patient.town')
                                                    ?: data_get($meta, 'customer.city')
                                                    ?: data_get($meta, 'customer.town');

                                                $postcode = data_get($meta, 'postcode')
                                                    ?: data_get($meta, 'post_code')
                                                    ?: data_get($meta, 'user.postcode')
                                                    ?: data_get($meta, 'user.post_code')
                                                    ?: data_get($meta, 'patient.postcode')
                                                    ?: data_get($meta, 'patient.post_code')
                                                    ?: data_get($meta, 'customer.postcode')
                                                    ?: data_get($meta, 'customer.post_code');

                                                // If still blank, fall back to real order user
                                                if ((!$addr1 || trim((string) $addr1) === '') && ($record->reference ?? null)) {
                                                    try {
                                                        $order = \App\Models\Order::where('reference', $record->reference)->latest()->first();
                                                        if ($order && isset($order->user)) {
                                                            $addr1 = $addr1 ?: ($order->user->address1 ?? null);
                                                            $addr2 = $addr2 ?: ($order->user->address2 ?? null);
                                                            $city = $city ?: ($order->user->city ?? null);
                                                            $postcode = $postcode ?: ($order->user->postcode ?? null);
                                                        }
                                                    } catch (\Throwable $e) {
                                                    }
                                                }
                                                ...
                                                $parts = [];
                                                foreach ([$addr1, $addr2, $city, $postcode] as $p) {
                                                    if (is_scalar($p)) {
                                                        $t = trim((string) $p);
                                                        if ($t !== '') $parts[] = $t;
                                                    }
                            ...
                            ->schema([
                                ViewEntry::make('order_history_view')
                                    ...
                                    ->getStateUsing(function ($record) {
                                        ...
                                        $uid = $record->user_id ?? null ?: optional($record->user)->id;
                    ...
                    ->extraModalFooterActions([
                        Action::make('completeConsultation')
                            ...
                            ->modalHeading('Complete NHS consultation')
                            ->modalDescription('This will store the NHS order as completed and close it out from pending.')
                            ->action(function (NhsPending $record, Action $action) {
                                $meta = is_array($record->meta)
                                    ? $record->meta
                                    : (json_decode($record->meta ?? '[]', true) ?: []);

                                $meta['nhs_pending_status'] = 'completed';

                                data_set($meta, 'payment_status', 'free');
                                data_set($meta, 'payment_status_label', 'Free');
                                data_set($meta, 'total_minor', 0);
                                data_set($meta, 'type', data_get($meta, 'type') ?: 'nhs');
                                ...
                                if (! $order) {
                                    ...
                                    $order->reference = $record->reference;
                                }
                                ...
                                return redirect(\App\Filament\Resources\NhsPending\Pages\ListNhsPending::getUrl());
                            }),

                    Action::make('rejectConsultation')
                        ->label('Reject')
                        ->color('danger')
                        ->icon('heroicon-o-x-circle')
                        ->requiresConfirmation()
                        ->modalHeading('Reject NHS consultation')
                        ->modalDescription('This will mark the NHS order as rejected and remove it from the pending list.')
                        ->action(function (NhsPending $record, Action $action) {
                            $meta = is_array($record->meta)
                                ? $record->meta
                                : (json_decode($record->meta ?? '[]', true) ?: []);

                            $meta['nhs_pending_status'] = 'rejected';
                            data_set($meta, 'type', data_get($meta, 'type') ?: 'nhs');
                        ...
                        Action::make('addAdminNote')
                            ...
                            ->action(function (NhsPending $record, array $data, Action $action, $livewire) {
                                ...
                                $livewire->dispatch('close-modal', id: 'mountedTableAction');
