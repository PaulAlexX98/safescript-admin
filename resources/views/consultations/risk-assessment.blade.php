...
@php
    ...
    $answerFor = function ($name, $label = null) use ($oldData, $prefill, $flatIndexByKey) {
        ...
        // Fuzzy fallback  token based  handles small schema key drifts like feed vs feeding  and  details suffixes
            ...
            foreach ($candidates as $cand) {
                ...
                foreach ($wantTokens as $t) {
                    foreach ($tokensC as $tc) {
                        ...
                        // prefix match of length >= 3 to catch feeding vs feed
...
@else
    <form id="cf_risk-assessment" method="POST" action="{{ route('consultations.forms.save', ['session' => $session->id, 'form' => $form->id]) }}?tab=risk-assessment">
        @csrf
